                    ...
                    repeat
                        ...
                        if rec."Currency Factor" <> 0 then begin
                            ...
                            if rec."LFS FOB CIF Calc. Type" = rec."LFS FOB CIF Calc. Type"::FOB then begin
                                ...
                                SalesLine."LFS CIF(FCY)" := SalesLine."LFS Freight Value (FCY)" + SalesLine."LFS Insurance Value (FCY)" + SalesLine."LFS FOB Amount (FCY)";
                                SalesLine."LFS CIF(LCY)" := SalesLine."LFS CIF(FCY)" * exchRate;
                                if SalesLine.Quantity <> 0 then begin
                                    SalesLine."LFS FOB Amount (FCY)" := SalesLine."LFS FOB Amount (FCY)" / SalesLine.quantity;
                                    SalesLine."LFS CIF Amount (FCY)" := SalesLine."LFS CIF(FCY)" / SalesLine.Quantity;
                                end;
                            end
                            else
                                if rec."LFS FOB CIF Calc. Type" = rec."LFS FOB CIF Calc. Type"::CIF then begin
                                    SalesLine."LFS Freight Value (LCY)" := SalesLine."LFS Freight Value (FCY)" * exchRate;
                                    SalesLine."LFS Insurance Value (LCY)" := SalesLine."LFS Insurance Value (FCY)" * exchRate;
                                    SalesLine."LFS CIF(FCY)" := SalesLine."Line Amount";
                                    SalesLine."LFS CIF(LCY)" := SalesLine."Line Amount" * exchRate;
                                    SalesLine."LFS FOB Amount (FCY)" := SalesLine."LFS CIF(FCY)" - SalesLine."LFS Freight Value (FCY)" - SalesLine."LFS Insurance Value (FCY)";
