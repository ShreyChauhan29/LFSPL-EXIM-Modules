report 72017 "LFSExport Comm Draft Pack List"
{
    ApplicationArea = All;
    Caption = 'Custom Dispatch Packing List';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './SRC/Reports Layouts/Export Commercial Draft Packing List Report.rdl';
    dataset
    {
        dataitem(SalesHeader; "Sales Header")
        {
            ...
            column(CompanyInfo_IECNo; ' CompanyInfo."LFS_EX_I.E.C. CODE NO."')
            {
            }
            ...
            column(RBICODENO; 'CompanyInfo."LFS_EX_R.B.I. CODE NO."')
            {

            }
            column(IECCodeNo; 'CompanyInfo."LFS_EX_I.E.C. CODE NO."')
            {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                { }
                column(LFS_EX_to; 'LFS_EX_to')
                {

                }
                column(LFS_EX_From; 'LFS_EX_From')
                {
                ...
                dataitem(ReservationEntrydataitem; "Reservation Entry")
                {
                    ...
                    trigger OnAfterGetRecord()
                    begin
                        if MonthYear = false then begin
                            // MonthYearMFG := Format(ReservationEntrydataitem."LFS Mfg. Date", 0, '<Day,2>-<Month,2>-<Year4>');
                            MonthYearEXP := Format(ReservationEntrydataitem."Expiration Date", 0, '<Day,2>-<Month,2>-<Year4>');
                        end else begin
                            // MonthYearMFG := Format(ReservationEntrydataitem."LFS Mfg. Date", 0, '<Month Text> <Year4>');
                            MonthYearEXP := Format(ReservationEntrydataitem."Expiration Date", 0, '<Month Text> <Year4>');
                        end
                    end;
                }
                // dataitem("LFS Packing List"; "LFS Packing List")
                // {
                //     DataItemLinkReference = "Sales Line";
                //     DataItemLink = "LFS_EX_Posted Export Shipment" = field("Shipment No."), "LFS_EX_PostExShipmentLineNo." = field("Shipment Line No.");
                //     column(BatchNo; "LFS_EX_Lot No.")
                //     { }
                //     column(LFS_EX_Line_No_; "LFS_EX_Line No.")
                //     {
                //     }
                //     column(LFS_EX_Tare_Weight_of_Pack; "LFS_EX_Tare Weight of Pack")
                //     {
                //     }
                //     column(GrossWeight; "LFS_EX_Gross Weight of Pack")
                //     {
                //     }
                //     column(No____Kind_Of_PKDG; PackingNo)
                //     {
                //     }
                //     column(LFS_EX_Net_Weight_of_Pack; "LFS_EX_Net Weight of Pack")
                //     {

                //     }
                //     column(LFS_EX_Tare_Weight_of_Pallet; "LFS_EX_Tare Weight of Pallet")
                //     {
                ...
                //     begin
                //         // ILE.Reset();
                //         // ILE.SetRange("Item No.", "LFS Packing List"."LFS_EX_Item Code");
                //         // ILE.SetRange("Variant Code", "LFS Packing List"."LFS_EX_Variant Code");
                //         // ILE.SetRange("Location Code", "LFS Packing List"."LFS_EX_Location Code");
                //         // ILE.SetRange("Document No.", "LFS Packing List"."LFS_EX_Posted Export Shipment");
                //         // ILE.SetRange("Document Line No.", "LFS Packing List"."LFS_EX_PostExShipmentLineNo.");
                ...
                begin
                    ...
                    if SalesHeader."Document Type" = SalesHeader."Document Type"::Invoice then begin
                        ...
                        RecPackingList.SetRange("LFS Posted Export Shipment", "Sales Line"."Shipment No.");
                        RecPackingList.SetRange("LFS PostExShipmentLineNo.", "Sales Line"."Shipment Line No.");
                        RecPackingList.SetRange("LFS Item Code", "Sales Line"."No.");
                        RecPackingList.SetRange("LFS Variant Code", "Sales Line"."Variant Code");
                        ...
                        if SalesHeader."Document Type" = SalesHeader."Document Type"::Order then begin
                            PackingList1.Reset();
                            PackingList1.SetRange("LFS Source Document No.", "Sales Line"."Document No.");
                            PackingList1.SetRange("LFS Source Doc. Line No.", "Sales Line"."Line No.");
                            if PackingList1.Findset() then
                                repeat
                                    NetWeight := PackingList1."LFS Net Weight of Pack";
                                    GrossWeightPack := PackingList1."LFS Gross Weight of Pack";
                                    TotalNetWeight += NetWeight;
                                    TotalGrossWeight += GrossWeightPack
                                until PackingList1.Next() = 0;
                        end;
            ...
            begin
                ...
                if PostedExportInformation.FindFirst() then begin
                    ShipBillNo := SalesHeader."LFS Shipping Bill No.";
                    ShipBillDate := SalesHeader."LFS Shipping Bill Date";
                    ...
                    Delivery_Terms := PostedExportInformation."LFS Terms of Delivery";
                    // PaymentTerms2 := PostedExportInformation."LFS Payment Terms 2";
                    PortName := SalesHeader."LFS Port of Discharge";
                    PortOfLoading := SalesHeader."LFS Port of Loading";
                    BuyerIfotherthenConsignee_ := PostedExportInformation."LFS Consignee Name & Address";
    ...
    var
        ...
        TermsDescription, BankAccNo, paymenttermname : Text;
        ...
        Consignee_Address, CartonDimensions : Text[500];
        ...
        LUT_Date, CustomerOrderDate : Date;
        Bill_to_County, ShiptoCoutry, BilltoCountry : Text[50];
        LocState: Text;
        CurrDes: text;
        TotalNetWeight, TotalGrossWeight : Decimal;
    ...
    // NetWt: Decimal;
    // PackingType: Code[10];
    // EximLicHeader: Record "LFS EXIM  License Header";
    // LicDate: date;
    // LicFileNo: Text;
    // BatchNo: code[50];
    // Quantity1: Decimal;
    ...
    // PosPackList: Record "LFS Posted Packing List";
